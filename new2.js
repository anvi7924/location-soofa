...
$( document ).ready(function() {
	...
	output = "var walkscoreData ={max: max, data: [";
	for (i = 0; i < num_x; i++) {
		for (j = 0; j < num_y; j++) {
			lat = yvals[j];
			lng = xvals[i];
			myURL = "http://api.walkscore.com/score?format=json&lat=" + lat + "&lon=" + lng + "&wsapikey=f0bca1eef985b837c171e342411aa3d6";
			finalURL = proxy + myURL;
			$.getJSON(finalURL, function( data ) {
		    	output += "{lat: " + lat +  ", lng: " + lng + ", count: " + data.walkscore + "}";
			    $("#layerselector").append("<div id='walkscore'>" + output + "</div>");
			    $("#walkscore").remove();
		    });
